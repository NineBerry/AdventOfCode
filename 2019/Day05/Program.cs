...
{
#if Sample
    string fileName = @"D:\Dropbox\Work\AdventOfCode\2019\Day05\Sample.txt";
    int part2Input = 9;
#else
    string fileName = @"D:\Dropbox\Work\AdventOfCode\2019\Day05\Full.txt";
    int part2Input = 5;
#endif

    int[] values = File.ReadAllText(fileName).Split(',').Select(int.Parse).ToArray();

    Console.WriteLine("Part 1: " + Solve(values, 1));
    Console.WriteLine("Part 2: " + Solve(values, part2Input));
    Console.ReadLine();
}

int Solve(int[] values, int input)
{
    Computer computer = new Computer(values);
    int output = 0;
